...
The marketplace uses two database tables that are automatically created:

### ProductCardCategory

Defines filterable categories:

```sql
CREATE TABLE ProductCardCategory (
...
```sql
CREATE TABLE AnalyticsMarketplaceImpressionsClicks (
    Id BIGINT PRIMARY KEY IDENTITY,
    CompanyId BIGINT NULL,
    LocationId BIGINT NULL,
    ProductCardId BIGINT,
    IsImpression BIT,
    IsClick BIT,
...
- Verify `ProductCardCategory` entries exist in database
- Ensure `PropertyName` matches product model properties
- Check `showFilters` prop is set to `true`

### Analytics not tracking

- Verify `AnalyticsMarketplaceImpressionsClicks` table exists
